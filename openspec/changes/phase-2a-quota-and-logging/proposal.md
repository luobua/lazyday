...
**Backend — Phase 1 注册流程修改**
- 修改 `AuthFacadeImpl.register(...)`：在同一事务内增加"为新租户绑定默认 Free 套餐"步骤（`t_tenant` + `t_user` + `t_tenant_quota` 三表原子写入），保证"每个租户恰有一条 quota 记录"不变量在新租户场景也成立
...
- **Backend API**：新增 `/api/portal/v1/{quota,logs,logs/stats}` + `/api/admin/v1/{plans,tenants/{id}/quota}` + `/internal/v1/quota/effective`
